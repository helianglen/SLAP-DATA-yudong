...
%az = 360 - azAll; --YDT done in combineAz.m
az=azAll; 
...
%YDT: of no noise-corrected data, use original 
%    else
...
% calibration 
    Tbh = (havg - 2507448.51) / 4152.4488;
    Tbv = (vavg - 2306412.72) / 4376.8796;

figure
subplot(3, 1, 1)
plot(timeavg, Tbh, 'g')
axis([min(timeavg) max(timeavg) 50 250])
...
subplot(3, 1, 2)
plot(timeavg, Tbv-Tbh, 'Color', [0 0 0])
axis([min(timeavg) max(timeavg) -50 40])
title('TbV - TbH (K)')
datetick('x', 15)
hold on
plot(timeavg, zeros(length(timeavg)), 'Color', 'g')

subplot(3, 1, 3)
plot(timeavg, rollavg, 'Color', [0 0 0])
axis([min(timeavg) max(timeavg) -50 10])
